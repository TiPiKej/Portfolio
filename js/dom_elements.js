let dom = {
			...
			to_scroll = setTimeout(() => {
				let linki = document.querySelectorAll('nav a');
					...
					for(let i = linki.length; i > 0; i--){
						if(window.top.scrollY - document.querySelector(linki[i-1].hash).offsetParent.offsetTop >= -(window.top.innerHeight / 4)){
	...
	and: (click, e = 'nope', i) => {
		...
		if(click.target.tagName.toLowerCase() == 'div') click.target.appendChild(effect);
		else click.target.parentNode.appendChild(effect);
		setTimeout(function() {
		if(click.target.tagName.toLowerCase() == 'div') click.target.removeChild(effect);
										...
										jezyk.onmousemove = event => {
											...
											to_jezyk = setTimeout( () => {
												if(document.querySelectorAll(`.${jezyk_info}`).length > 0){
													...
													info.addEventListener('mousemove', event => event.stopPropagation());
												}
											...
											window.addEventListener('scroll', () => Usun(document.querySelectorAll(`.jezyk_info`)));

											window.addEventListener('mousemove', () => Usun(document.querySelectorAll(`.jezyk_info`)));
										}
	...
	afterLoading: (what = window.location.hash) => {
		if(what == '') what = '#header';
		window.scrollBy({
			top: document.querySelector(what).offsetParent.offsetTop - window.top.scrollY,
		  left: 0, 
		  behavior: 'smooth'
		});
	}
